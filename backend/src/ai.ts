 ...
 */
async function retrieveRelevantChunks(userId: string, vaultId: string, message: string): Promise<DocumentChunk[]> {
  ...
  try {
    ...
    if (!response.ok) {
      const fallbackResponse = await fetch(`${SUPABASE_URL}/rest/v1/document_chunks?user_id=eq.${userId}&vault_id=eq.${vaultId}&limit=8`, {
        headers: {
          'apikey': SUPABASE_SERVICE_KEY,
          'Authorization': `Bearer ${SUPABASE_SERVICE_KEY}`
        }
...
async function getAllVaultChunks(userId: string, vaultId: string): Promise<DocumentChunk[]> {
  ...
  try {
    ...
    const response = await fetch(`${SUPABASE_URL}/rest/v1/document_chunks?user_id=eq.${userId}&vault_id=eq.${vaultId}`, {
      headers: {
        'apikey': SUPABASE_SERVICE_KEY,
        'Authorization': `Bearer ${SUPABASE_SERVICE_KEY}`
      }
...
async function getChatHistory(conversationId: string): Promise<ChatMessage[]> {
  if (!SUPABASE_URL || !SUPABASE_SERVICE_KEY || !conversationId) return [];
  ...
  try {
    ...
    const response = await fetch(`${SUPABASE_URL}/rest/v1/messages?conversation_id=eq.${conversationId}&order=created_at.asc&limit=10`, {
      headers: {
        'apikey': SUPABASE_SERVICE_KEY,
        'Authorization': `Bearer ${SUPABASE_SERVICE_KEY}`
      }
...
function buildChatPrompt(persona: LearningPersona | null, chunks: DocumentChunk[], message: string, history: ChatMessage[] = []): string {
  ...
  if (persona) {
    ...
    if (persona.error_correction) prompt += `- Preferred correction style: ${persona.error_correction.answer || persona.error_correction}\n`;
    if (persona.logic_structure) prompt += `- Preferred logical structure: ${persona.logic_structure.answer || persona.logic_structure}\n`;
    if (persona.abstraction_level) prompt += `- Preferred abstraction level: ${persona.abstraction_level.answer || persona.abstraction_level}\n`;
