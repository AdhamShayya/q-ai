...
async function updateDocumentStatus(
  ...
  try {
    ...
    const response = await fetch(`${SUPABASE_URL}/rest/v1/documents?id=eq.${documentId}`, {
      method: "PATCH",
      headers: {
    ...
    if (response.ok === false) {
      ...
      console.error(`[Processor] Failed to update document status: ${errorText}`)
    }
  } catch (error) {
    console.error(
 ...
 */
function buildMetadata(jobData: IngestionJobData, chunkIndex: number, totalChunks: number) {
  return {
...
export async function processIngestionJob(
  ...
  try {
    ...
    console.log(`[Processor] AI extracted ${extractedText.length} characters`)
    ...
    const metadatas = chunks.map((chunk) => buildMetadata(jobData, chunk.index, chunks.length))
    ...
    console.error(`[Processor] ❌ Ingestion failed for ${filename}: ${errorMessage}`)
